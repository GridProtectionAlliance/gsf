...
namespace GrafanaAdapters.GrafanaFunctions
{
    ...
    /// </remarks>
    public class Top : IGrafanaFunction
    {
        /// <inheritdoc />
        public string Name { get; } = "Top";

        /// <inheritdoc />
        public string Description { get; } = "Returns a series of N, or N% of total, values that are the largest in the source series.";

        /// <inheritdoc />
        public Type Type { get; } = typeof(Top);

        /// <inheritdoc />
        public Regex Regex { get; } = new Regex(string.Format(FunctionsModelHelper.ExpressionFormat, "(Top|Largest)"), RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        public DataSourceValueGroup<DataSourceValue> Compute(List<IParameter> parameters)
        {
            ...
            .OrderByDescending(dataSourceValue => dataSourceValue.Value)
        ...
        public DataSourceValueGroup<PhasorValue> ComputePhasor(List<IParameter> parameters)
        {
            ...
            IEnumerable<PhasorValue> transformedPhasorValues = phasorValues.Source
                .OrderByDescending(phasorValue => phasorValue.Magnitude)
                .Take(numberRequested);
