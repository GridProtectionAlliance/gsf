...
public interface IGrafanaFunction
{
    ...
    /// </para>
    /// </remarks>
    ParameterDefinitions Parameters { get; } // List of ParameterDefinition<T> objects
    ...
    /// <remarks>
    /// Using the same set of function parameters with mutable values would create race conditions for desired values
    /// when multiple threads are evaluating the same Grafana function. This function creates a new list of value
    /// mutable parameters from the defined parameter definitions in the <see cref="Parameters"/> property.
    /// </remarks>
    List<IParameter> GetValueMutableParameters(); // List of ValueMutableParameter<T> objects

    /// <summary>
    /// Gets the number of required parameters of the Grafana function.
    /// </summary>
...
public interface IGrafanaFunction<out T> : IGrafanaFunction where T : struct, IDataSourceValue<T>
{
    ...
    IEnumerable<T> Compute(List<IParameter> parameters, CancellationToken cancellationToken);
    ...
    /// </remarks>
    IEnumerable<T> ComputeSlice(List<IParameter> parameters, CancellationToken cancellationToken);
    ...
    /// <see cref="Compute"/> to perform the computation.
    /// </remarks>
    IEnumerable<T> ComputeSet(List<IParameter> parameters, CancellationToken cancellationToken);
}