...
namespace PowerCalculations.PowerMultiCalculator
{
	...
	public static class PowerCalculationConfigurationValidation
	{
			...
			using (var cmd = database.Connection.CreateCommand())
			{
				...
				using (var rdr = cmd.ExecuteReader())
				{
					while (rdr.Read())
					{
						...
						var companyAcronym = rdr.GetString(7);
						var deviceAcronym = rdr.GetString(6);
						var vendorAcronym = rdr.GetString(5);
						var signalTypeAcronym = "CALC";
						var circuitDescription = rdr.GetString(1);
						var deviceId = rdr.GetInt32(8);
						var historianId = rdr.GetInt32(9);

						if (rdr.IsDBNull(2))
						{
							// create real power output measurement
							var measurement = CreateMeasurement(companyAcronym, deviceAcronym, vendorAcronym, signalTypeAcronym, circuitDescription, deviceId, historianId, " Real Power Calculation");
						...
						if (rdr.IsDBNull(3))
						{
							...
							var measurement = CreateMeasurement(companyAcronym, deviceAcronym, vendorAcronym, signalTypeAcronym, circuitDescription, deviceId, historianId, " Active Power Calculation");
						...
						if (rdr.IsDBNull(4))
						{
							...
							var measurement = CreateMeasurement(companyAcronym, deviceAcronym, vendorAcronym, signalTypeAcronym, circuitDescription, deviceId, historianId, " Reactive Power Calculation");
		...
		{
			if (realPowerOutputSignalId == null && reactivePowerOutputSignalId == null && activePowerOutputSignalId == null) return;
