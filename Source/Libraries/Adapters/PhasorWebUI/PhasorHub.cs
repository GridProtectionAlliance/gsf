...
namespace PhasorWebUI
{
    ...
    public class PhasorHub : RecordOperationsHub<PhasorHub>
    {
        ...
        public void ValidateCalculatorConfigurations(int? historianID)
        {
            ...
            avgFreqAdapter.ConnectionString = $"InputMeasurementKeys={{FILTER ActiveMeasurements WHERE SignalType = 'FREQ'}}; OutputMeasurements={{{measurements[Avg].SignalID};{measurements[Max].SignalID};{measurements[Min].SignalID}}}; LagTime={lagTime}; LeadTime={DefaultCalculationLeadTime}; FramesPerSecond={DefaultCalculationFramesPerSecond}";
        ...
        private Measurement[] GetCalculatedFrequencyMeasurements(int? historianID)
        {
            ...
            Measurement avgFreqMeasurement = QueryMeasurement($"{SystemFrequencyDeviceName}-AVG-FQ");
            Measurement maxFreqMeasurement = QueryMeasurement($"{SystemFrequencyDeviceName}-MAX-FQ");
            Measurement minFreqMeasurement = QueryMeasurement($"{SystemFrequencyDeviceName}-MIN-FQ");

            avgFreqMeasurement.PointTag = $"{SystemFrequencyDeviceName}-AVG-FQ";
            avgFreqMeasurement.SignalReference = SignalReference.ToString(SystemFrequencyDeviceName, SignalKind.Frequency);
            ...
            maxFreqMeasurement.SignalReference = SignalReference.ToString(SystemFrequencyDeviceName, SignalKind.Analog, 1);
            ...
            minFreqMeasurement.SignalReference = SignalReference.ToString(SystemFrequencyDeviceName, SignalKind.Analog, 2);
            ...
            measurements[0] = QueryMeasurement($"{SystemFrequencyDeviceName}-AVG-FQ");
            measurements[1] = QueryMeasurement($"{SystemFrequencyDeviceName}-MAX-FQ");
            measurements[2] = QueryMeasurement($"{SystemFrequencyDeviceName}-MIN-FQ");
