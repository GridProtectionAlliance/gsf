    ...
    public class PhasorHub : RecordOperationsHub<PhasorHub>
    {
        ...
        public IEnumerable<Device> QueryChildDevices(int deviceID)
        {
            ...
            return DataContext.Table<Device>()
                .QueryRecordsWhere("ParentID = {0} OR (ParentID IS NULL AND ConnectionString LIKE {1})", deviceID, $"%{DetachedDeviceLink.ParentIDKey}={deviceID}%")
                .Where(device => device.ParentID == deviceID || DetachedDeviceLink.TryParseParentID(device.ConnectionString, out int parentID) && parentID == deviceID);
        }
        ...
        public ConfigurationFrame ExtractConfigurationFrame(int deviceID)
        {
            ...
            // Use effective parent ID so a detached child, i.e., a child modeled as a standalone device
            // with the parent linkage in its connection string, is still derived as a concentrator child
            if (DetachedDeviceLink.GetEffectiveParentID(device.ParentID, device.ConnectionString) == null)
            {
