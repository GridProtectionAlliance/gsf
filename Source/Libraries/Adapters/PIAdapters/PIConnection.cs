...
public class PIConnection : IComparable<PIConnection>, IComparable, IDisposable
{
    ...
    public void Open()
    {
        ...
        {
            throw new InvalidOperationException($"Failed to connect to PI server \"{ServerName}\": {ex.Message}");
        }
    ...
    {
        return other is null ? 1 : Comparer<Guid>.Default.Compare(m_instanceID, other.m_instanceID);
    }

    int IComparable.CompareTo(object obj)
    {
        if (obj is PIConnection other)
