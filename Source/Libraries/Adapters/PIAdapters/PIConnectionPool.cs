    ...
    public PIConnection GetPooledConnection(string serverName, string userName = null, string password = null, int connectTimeout = PIConnection.DefaultConnectTimeout)
    {
        ...
        try
        {
            lock (m_connectionPool)
            {
                ...
                {
                    // Get next connection from the pool with the lowest accessibility count
                ...
                Interlocked.Increment(ref connection.AccessCount);
            }
        }
        catch (Exception ex)
        {
            throw new InvalidOperationException($"Failed to get a pooled PI connection: {ex.Message}", ex);
        }
    ...
    public void ReturnPooledConnection(PIConnection connection)
    {
        ...
        lock (m_connectionPool)
        {
            ...
            if (connection.AccessCount > 0)
                Interlocked.Decrement(ref connection.AccessCount);
