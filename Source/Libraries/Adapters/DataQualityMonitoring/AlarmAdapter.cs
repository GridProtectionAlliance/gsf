...
namespace DataQualityMonitoring
{
    ...
    public class AlarmAdapter : FacileActionAdapterBase
    {
        ...
        public ICollection<Alarm> GetHighestSeverityAlarms()
        {
            ...
            {
                return m_alarmLookup
                    .Select(kvp => kvp.Value.FirstOrDefault(alarm => alarm.State == AlarmState.Raised))
        ...
        private void UpdateAlarmDefinitions()
        {
            ...
            Dictionary<Guid, List<Alarm>> newAlarmLookup;
            ...
            foreach (Alarm existingAlarm in m_alarmLookup.SelectMany(kvp => kvp.Value))
            {
                Alarm definedAlarm;
            ...
            newAlarmLookup = definedAlarmsLookup.Values
                ...
                .ToDictionary(grouping => grouping.Key, grouping => grouping.OrderByDescending(alarm => alarm.Severity).ThenBy(alarm => alarm.ID).ToList());
            ...
            try
            {
                ...
                foreach (KeyValuePair<Guid, List<Alarm>> kvp in m_alarmLookup)
                {
                    List<Alarm> existingAlarms = kvp.Value;
                    List<Alarm> definedAlarms;

                    // Get the active alarms from both before and after the configuration changes were applied
                    Alarm existingActiveAlarm = existingAlarms.FirstOrDefault(alarm => alarm.State == AlarmState.Raised);
                    Alarm definedActiveAlarm = null;

                    if (newAlarmLookup.TryGetValue(kvp.Key, out definedAlarms))
                        definedActiveAlarm = definedAlarms.FirstOrDefault(alarm => alarm.State == AlarmState.Raised);
            ...
            try
            {
                foreach (IMeasurement measurement in measurements)
                {
                    lock (m_alarmLock)
                    {
                        ...
                        activeAlarm = alarms.FirstOrDefault(alarm => alarm.State == AlarmState.Raised);

                        // Test each alarm to determine whether their states have changed
                        alarmEvents.AddRange(alarms.Where(alarm => alarm.Test(measurement)).Select(alarm => CreateAlarmEvent(measurement.Timestamp, alarm)));

                        // Get the alarm that will become the currently active alarm
                        firstRaisedAlarm = alarms.FirstOrDefault(alarm => alarm.State == AlarmState.Raised);
                    }
