...
public class ArchiveReader : IDisposable
{
    ...
    /// </summary>
    public string FileName => m_archiveFile?.FileName;

    /// <summary>
    /// Gets the path to the offload location used by the <see cref="ArchiveFile"/>.
    /// </summary>
    public string ArchiveOffloadLocation => m_archiveFile?.ArchiveOffloadLocation;

    /// <summary>
    /// Gets the <see cref="StateFile"/> used by the <see cref="ArchiveFile"/>.
    /// </summary>
    public StateFile StateFile => m_archiveFile?.StateFile;

    /// <summary>
    /// Gets the <see cref="MetadataFile"/> used by the <see cref="ArchiveFile"/>.
    /// </summary>
    public MetadataFile MetadataFile => m_archiveFile?.MetadataFile;

    /// <summary>
    /// Gets the <see cref="IntercomFile"/> used by the <see cref="ArchiveFile"/>.
    /// </summary>
    public IntercomFile IntercomFile => m_archiveFile?.IntercomFile;

    /// <summary>
    /// Gets the descriptive status of the internal <see cref="ArchiveFile"/>.
    /// </summary>
    public string Status => m_archiveFile is null ? "" : m_archiveFile.Status;
    ...
    protected virtual void Dispose(bool disposing)
    {
        ...
        try
        {
            ...
            if (m_rolloverWatcher is not null)
            {
    ...
    private void m_rolloverWatcher_Elapsed(object sender, ElapsedEventArgs e)
    {
        ...
        try
        {
            ...
            if (m_archiveFile?.IntercomFile is not { IsOpen: true })
                return;
