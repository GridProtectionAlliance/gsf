...
public class TableOperations<T> : ITableOperations where T : class, new()
{
    ...
    public T[] SearchRecords(string sortField, bool ascending, string searchText, StringComparison comparison = StringComparison.OrdinalIgnoreCase)
    {
        ...
        string[] searchValues = searchText.RemoveDuplicateWhiteSpace().Split([' '], StringSplitOptions.RemoveEmptyEntries);
        bool sortFieldIsEncrypted = FieldIsEncrypted(sortField);
    ...
    public RecordRestriction GetSearchRestriction(string searchText)
    {
        ...
        for (int i = 0; i < searchValues.Length; i++)
        {
            ...
            foreach (PropertyInfo property in s_encryptedSearchTargets)
            {
                ...
                parameters.Add(s_encryptDataTargets.TryGetValue(property, out string keyReference) ? 
                    searchValues[i].Encrypt(keyReference, CipherStrength.Aes256) : 
                    null);
            }
    ...
    {
        if (!s_enableSecureOrderBy || string.IsNullOrWhiteSpace(orderByExpression))
        ...
        foreach (string fieldExpression in fieldExpressions)
        {
            ...
            {
                fieldName = trimmedExpression.Substring(0, trimmedExpression.Length - 4).Trim();
                direction = " ASC";
            }
            else if (trimmedExpression.EndsWith(" DESC", StringComparison.OrdinalIgnoreCase))
            {
                fieldName = trimmedExpression.Substring(0, trimmedExpression.Length - 5).Trim();
                direction = " DESC";
            }
            else
            {
    ...
    private static readonly bool s_enableSecureOrderBy;
    ...
    static TableOperations()
    {
        ...
        foreach (PropertyInfo property in s_properties.Values)
        {
            ...
            if (searchableAttribute is not null)
            {
                ...
                {
                    searchFilterSql.Append(property.PropertyType == typeof(string) ? 
                        $"{fieldName} LIKE {{0}}" : 
                        $"{fieldName}={{1}}");
                }
                else
                {
                    searchFilterSql.Append(searchableAttribute.SearchType == SearchType.LikeExpression ? 
                        $"{fieldName} LIKE {{0}}" : 
                        $"{fieldName}={{1}}");
                }
        ...
        // TODO: Change this to default to true once remaining applications are migrated
        const bool DefaultEnableSecureOrderBy = false;

        // Load configured setting for secure order by
        try
        {
            ConfigurationFile configFile = ConfigurationFile.Current;

            CategorizedSettingsElementCollection systemSettings = configFile.Settings[nameof(systemSettings)];
            systemSettings.Add("EnableSecureOrderBy", DefaultEnableSecureOrderBy, "Enables validated security in 'ORDER BY' expressions used by table operations.");

            s_enableSecureOrderBy = systemSettings["EnableSecureOrderBy"].ValueAs(DefaultEnableSecureOrderBy);
        }
        catch (Exception ex)
        {
            Logger.SwallowException(ex, nameof(TableOperations<T>), $"Failed to load 'systemSettings.EnableSecureOrderBy' setting, defaulting to {DefaultEnableSecureOrderBy}");
            s_enableSecureOrderBy = DefaultEnableSecureOrderBy;
        }
    }
    ...
    {
        using AdoDataConnection connection = new(null, typeof(NullConnection), typeof(DbDataAdapter));
    ...
    public static string GetTableName()
    {
        ...
        if (escapedTableNameTargets is not null)
            tableName = $"\"{tableName}\"";
