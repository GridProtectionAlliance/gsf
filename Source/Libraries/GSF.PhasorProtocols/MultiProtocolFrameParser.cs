...
public sealed class MultiProtocolFrameParser : IFrameParser
{
    ...
    private sealed class SharedTcpServerReference : IServer
    {
        ...
        public WaitHandle[] MulticastAsync(byte[] data, int offset, int length) => m_tcpServer is null ? [] : [m_tcpServer.SendToAsync(m_clientID, data, offset, length)];
    ...
    public bool AutoRepeatCapturedPlayback
    {
        ...
        set
        {
            ...
            if (fileClient.AutoRepeat && fileClient.CurrentState == ClientState.Connected && fileClient.AtEOF)
