...
internal class InputWizardDevices : PagedViewModelBase<InputWizardDevice, string>, IDataErrorInfo
{
        ...
        : base(itemsPerPage, autoSave)
    {
        ...
        HistorianID = HistorianLookupList.Count switch
        {
            > 1 => HistorianLookupList.Skip(1).First().Key,
            > 0 => HistorianLookupList.First().Key,
            _ => HistorianID
        };
    ...
    {
        ObservableCollection<InputWizardDevice> wizardDeviceList = [];
    ...
    {
        InputWizardWalkthrough walkthrough = new()
        {
    ...
    {
        ConnectionStringBuilder csb = new(ConnectionStringBuilder.ConnectionType.DeviceConnection);
    ...
    {
        ConnectionStringBuilder csb = new(ConnectionStringBuilder.ConnectionType.AlternateCommandChannel);
    ...
    private void RequestConfiguration()
    {
        ...
        timer.Tick += (_, _) => RequestConfigurationPopupText += ".";
        timer.Interval = TimeSpan.FromSeconds(1.0D);
        timer.Start();

        Thread requestConfigurationThread = new(() =>
        {
            ...
            try
            {
                ...
                if (windowsServiceClient?.Helper?.RemotingClient is { CurrentState: ClientState.Connected })
                {
    ...
    {
        foreach (string message in e.Argument2.Split([Environment.NewLine], StringSplitOptions.RemoveEmptyEntries))
        {
    ...
    public void SavePDC()
    {
        ...
        try
        {
            ...
            Device device = new()
            {
                ...
                HistorianID = HistorianID == 0 ? null : HistorianID,
                ProtocolID = ProtocolID == 0 ? null : ProtocolID,
                InterconnectionID = InterconnectionID == 0 ? null : InterconnectionID,
                VendorDeviceID = PdcVendorDeviceID == 0 ? null : PdcVendorDeviceID,
    ...
    {
        Mouse.OverrideCursor = Cursors.Wait;
        AdoDataConnection database = new(CommonFunctions.DefaultSettingsCategory);
        ...
        try
        {
            ...
            foreach (InputWizardDevice inputWizardDevice in ItemsSource)
            {
                if (inputWizardDevice.Include)
                {
                    ...
                    device.HistorianID = HistorianID == 0 ? null : HistorianID;
                    device.ProtocolID = ProtocolID == 0 ? null : ProtocolID;
                    device.InterconnectionID = InterconnectionID == 0 ? null : InterconnectionID;
                    ...
                    HashSet<InputWizardDevicePhasor> unsavedInputPhasorSet = [..inputWizardDevice.PhasorList];
                    IList<Phasor> oldPhasorList = Phasor.Load(database, Phasor.LoadKeys(database, device.ID));
                    HashSet<Phasor> unsavedOldPhasorSet = [..oldPhasorList];
