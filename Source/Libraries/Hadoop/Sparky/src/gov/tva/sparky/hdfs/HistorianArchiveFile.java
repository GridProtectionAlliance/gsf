...
import TVA.Hadoop.MapReduce.Historian.File.*;

/**
 * This class defines the basic functions for opening and parsing an openPDC historical file.
 * 
 * @author JPatterson
 * @version 0.1.0
 */
public class HistorianArchiveFile {
	...
	/**
	 * The constructor for HistorianArchiveFile.
	 * 
	 * @param fs A reference to the FileSystem.
	 * @param file_path The path of the file to open.
	 * @param conf A hadoop configuration object.
	 * @throws IOException
	 */
	public HistorianArchiveFile(FileSystem fs, Path file_path,
			Configuration conf) throws IOException
	{
		...
		this.in = openFile(fs, this.file, bufferSize);
	...
	/**
	 * Gets the file allocation table for this archive file.
	 * 
	 * @return Returns an ArhichiveFileAllocationTable object.
	 */
	public ArchiveFileAllocationTable GetFatTable()
	{
		return this._FAT;
	}

	private ArchiveFileAllocationTable ReadFAT()
	{
	...
	{
		// block map in bytes
		long lVarBlockMapSize = 10 + (this._FAT._EventBlockCount * 12);
	...
	/**
	 * Parses the points stored in a particular point block in this archive file.
	 * 
	 * @param iBlockIndex The index of the point block to parse.
	 * @return Returns an ArrayList of StandardPointFile objects.
	 * @throws IOException
	 */
	public ArrayList<StandardPointFile> ParseBlockInArchive(int iBlockIndex) throws IOException
	{
		byte[] bufPoint = new byte[10];
		ByteBuffer PointBuffer = ByteBuffer.wrap(bufPoint);
		PointBuffer.order(ByteOrder.LITTLE_ENDIAN);

		ArrayList<StandardPointFile> arLocalPoints = new ArrayList<StandardPointFile>();

		// seek to the offset
		this.seek(iBlockIndex * 8 * 1024);

		// now read all 819 points
		// TODO: remove this hardcoding
		for (int i = 0; i < 819; i++)
		{
			this.ReadPointAtCurrentPosition();
			arLocalPoints.add(this.currentPoint);
		}

		return arLocalPoints;
	}
	...
	private void ReadPointAtCurrentPosition() throws IOException
	{
		...
		ArchiveDataBlockPointer BlockPointer = this._FAT._BlockMap
				.GetBlockPointerByIndex(this.iCurrentBlockIndex);
