...
import TVA.Hadoop.MapReduce.Historian.File.ArchiveDataBlockPointer;
...
{
	// row keys to all pointers in a heap
	private HashMap<String, PriorityQueue<HDFSPointBlockPointer>> _keyMap;
	private int iArchiveFileName = -1;
	...
	public FileIndex(String inputFileName, int iIndexRegistryFileID ) throws IOException
	{
		_keyMap = new HashMap<String, PriorityQueue<HDFSPointBlockPointer>>();
		this.iArchiveFileName = iIndexRegistryFileID;
		
		makeBuckets(parseFile(inputFileName));
	}
	

	
	public static Long GetTimestamp( double dTime )
	{
		long lTest = (long) (1000 * dTime);
		
		return lTest + cBaseDateTime.getTimeInMillis();
	}	
	
	private HashMap<Integer, PriorityQueue<HDFSPointBlockPointer>> parseFile(String strInputFilename) throws IOException
	{
	...
	private void makeBuckets(HashMap<Integer, PriorityQueue<HDFSPointBlockPointer>> pointMap)
	{
		// for all keys
		for (Integer key : pointMap.keySet())
		{
			PriorityQueue<HDFSPointBlockPointer> pointHeap = pointMap.get(key);
			
			while(pointHeap.size() > 0)
			{
				HDFSPointBlockPointer pointer = pointHeap.remove();
				String indexKey = Index.CalculateIndexRowKeyString(key, pointer.start_ts);
				
				// ensure our composite key exists in keymap
				if( !_keyMap.containsKey(indexKey))
					_keyMap.put(indexKey, new PriorityQueue<HDFSPointBlockPointer>());
				
				// reference the heap at this key, add the point to it
				PriorityQueue<HDFSPointBlockPointer> bucketHeap = _keyMap.get( indexKey );				
				bucketHeap.add( pointer );
				
			}
		}
	}
	
	public HashMap<String, PriorityQueue<HDFSPointBlockPointer>> GetKeyMap()
	{
		return _keyMap;
	}
}