...
public enum FtpType
{
    ...
    /// </summary>
    [Description("FTP: Standard File Transfer Protocol")]
    Ftp,
    /// <summary>
...
public class FtpClient : IDisposable
{
    ...
    /// <summary>
    /// Defines the default value for the <see cref="Timeout"/> property.
    /// </summary>
    public const int DefaultTimeout = 30000;

    /// <summary>
    /// Defines the default value for the <see cref="CaseSensitive"/> property.
    /// </summary>
    public const bool DefaultCaseSensitive = false;

    /// <summary>
    /// Defines the default value for the <see cref="Passive"/> property.
    /// </summary>
    public const bool DefaultPassive = true;

    /// <summary>
    /// Defines the default value for the <see cref="TFtpDirectoryFile"/> property.
    /// </summary>
    public const string DefaultTFtpDirectoryFile = "DIR.TXT";
    ...
    /// </summary>
    public bool Passive { get; set; } = DefaultPassive;
    ...
    /// <remarks>
    /// <paramref name="userName"/> and <paramref name="password"/> values are ignored on TFTP.
    /// </remarks>
    public void Connect(string userName, string password)
    {
        switch (FtpType)
        {
            case FtpType.Ftp:
            {
                FtpClientConnection = new(!CaseSensitive)
                {
                    Server = Server,
                    Port = Port ?? 21,
                    Timeout = Timeout,
                ...
                RootDirectory = new NativeFtpDirectory(this, FtpClientConnection.RootDirectory);
            ...
            case FtpType.TFtp:
            {
                TFtpClientConnection = new(Server, Port ?? TFtp.TFtpServer.DefaultServerPort);
                CurrentDirectory = RootDirectory = new TFtpDirectory(this, TFtpDirectoryFile);
                break;
            }
    ...
    /// <remarks>
    /// <paramref name="directoryPath"/> value is ignored on TFTP.
    /// </remarks>
    ...
    internal void OnBeginFileTransfer(string localFileName, string remoteFileName, TransferDirection transferDirection) => 
        BeginFileTransfer?.Invoke(this, new(localFileName, remoteFileName, transferDirection));

    internal void OnEndFileTransfer(string localFileName, string remoteFileName, TransferDirection transferDirection) =>
        EndFileTransfer?.Invoke(this, new(localFileName, remoteFileName, transferDirection));

    internal void OnFileTransferProgress(ProcessProgress<long> fileTransferProgress, TransferDirection transferDirection) => 
        FileTransferProgress?.Invoke(this, new(fileTransferProgress, transferDirection));

    internal void OnFileTransferNotification(FtpTransferResult transferResult) => 
        FileTransferNotification?.Invoke(this, new(transferResult));

    internal void OnResponseReceived(string response) => 
        ResponseReceived?.Invoke(this, new(response));

    internal void OnCommandSent(string command) => 
        CommandSent?.Invoke(this, new(command));
    ...
    {
        FtpAsyncResult result = e.Argument;
        OnFileTransferNotification(new(result.Message, result.ResponseCode, FtpTransferResult.GetResult(result.IsSuccess, result.IsFailed, result.IsAborted)));
    }
