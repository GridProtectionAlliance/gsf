//******************************************************************************************************
//  SubscriberAdapter.java - Gbtc
//
//  Copyright © 2010, Grid Protection Alliance.  All Rights Reserved.
//
//  Licensed to the Grid Protection Alliance (GPA) under one or more contributor license agreements. See
//  the NOTICE file distributed with this work for additional information regarding copyright ownership.
//  The GPA licenses this file to you under the Eclipse Public License -v 1.0 (the "License"); you may
//  not use this file except in compliance with the License. You may obtain a copy of the License at:
//
//      http://www.opensource.org/licenses/eclipse-1.0.php
//
//  Unless agreed to in writing, the subject software distributed under the License is distributed on an
//  "AS-IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. Refer to the
//  License for the specific language governing permissions and limitations.
//
//  Code Modification History:
//  ----------------------------------------------------------------------------------------------------
//  04/16/2012 - Stephen C. Wills
//       Generated original version of source code.
//
//******************************************************************************************************

package org.gpa.tsf.transport.event;

/**
 * An abstract adapter class for receiving subscriber events. The
 * methods in this class are empty. This class exists as convenience
 * for creating listener objects.
 */
public class SubscriberAdapter implements SubscriberListener
{

	@Override
	public void statusMessageReceived(MessageEvent evt)
	{
	}

	@Override
	public void exceptionEncountered(MessageEvent evt)
	{
	}

	@Override
	public void dataStartTimeReceived(StartTimeEvent evt)
	{
	}

	@Override
	public void metadataReceived(MetadataEvent evt)
	{
	}

	@Override
	public void newMeasurementsReceived(MeasurementEvent evt)
	{
	}
	
	@Override
	public void processingCompleteCallback(MessageEvent evt)
	{
	}

	@Override
	public void connectionTerminated(MessageEvent evt)
	{
	}

}
