...
namespace PCS.IO.Compression.Zip.Algorithms.Streams 
{
	...
	public class InflaterInputBuffer
	{
		...
		public void Fill()
		{
			...
			while (toRead > 0) {
				...
				if ( count <= 0 ) {
					if (rawLength == 0) {
						throw new CompressionExceptionBase("Unexpected EOF"); 
					}
					break;
				}
				rawLength += count;
	...
	/// as the <see cref="PCS.IO.Compression.GZip.GZipInputStream">GZipInputStream</see>.
	///
	...
	public class InflaterInputStream : Stream
	{
		...
		public override int Read(byte[] buffer, int offset, int count) 
		{
			...
			{
				throw new CompressionExceptionBase("Need a dictionary");
			}
