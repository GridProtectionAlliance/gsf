...
using PCS.Security.Radius;
//using PCS.Security.Cryptography.Common;

//*******************************************************************************************************
//  PCS.Security.Application.User.vb - User defined in the security database
...
namespace PCS.Security
{
	namespace Application
	{
		...
		[Serializable()]public class User
		{
			...
			/// database. An external user is someone outside of PCS who does not have a PCS domain account.
			/// </summary>
			/// <value></value>
			...
			public void RefreshData()
			{
				...
				using (SqlConnection dbConnection = GetDatabaseConnection())
				{
					...
					userData = PCS.Data.Common.RetrieveDataSet("dbo.RetrieveApiData", dbConnection, m_username, m_applicationName);
				}
			...
			public bool Authenticate(string password)
			{
				...
				switch (m_authenticationMode)
				{
					case AuthenticationMode.AD:
						...
						else
						{
							...
							if (! string.IsNullOrEmpty(password))
							{
                                ...
                                m_isAuthenticated = new UserInfo("PCS", m_username, true).Authenticate(password);
							}
							else
							{
					...
					case AuthenticationMode.RSA:
						PCS.Security.Radius.RadiusClient client = null;
						RadiusPacket response = null;
						try
						{
							// We first try to authenticate against the primary RSA server.
							client = new PCS.Security.Radius.RadiusClient(RsaServer1, PCS.Security.Cryptography.Common.Decrypt(RadiusSharedSecret));
							...
							if (response == null)
							{
								...
								client.Dispose();
								client = new PCS.Security.Radius.RadiusClient(RsaServer2, PCS.Security.Cryptography.Common.Decrypt(RadiusSharedSecret));
								response = client.Authenticate(m_username, password);
							}
				...
				using (SqlConnection dbConnection = GetDatabaseConnection())
				{
					PCS.Data.Common.ExecuteNonQuery("dbo.LogLogin", dbConnection, m_username, ! m_isAuthenticated);
				}
				...
				switch (m_authenticationMode)
				{
					case AuthenticationMode.AD:
						...
						if (m_isExternal && Authenticate(oldPassword))
						{
							using (SqlConnection dbConnection = GetDatabaseConnection())
							{
								PCS.Data.Common.ExecuteScalar("dbo.ChangePassword", dbConnection, m_username, m_password, EncryptPassword(newPassword));
							}
			...
			public bool LogAccess(bool accessDenied)
			{
				...
				if (! string.IsNullOrEmpty(m_applicationName))
				{
					...
					using (SqlConnection dbConnection = GetDatabaseConnection())
					{
						PCS.Data.Common.ExecuteNonQuery("dbo.LogAccess", dbConnection, m_username, m_applicationName, accessDenied);
					}
			...
			public bool LogError(string source, string message)
			{
				...
				try
				{
					using (SqlConnection dbConnection = GetDatabaseConnection())
					{
						PCS.Data.Common.ExecuteScalar("dbo.LogError", dbConnection, m_applicationName, source, message);
					}
			...
			public static string GeneratePassword(int length)
			{
				...
				{
					char[] password = PCS.Common.CreateArray<char>(length);
					...
					for (int i = 0; i <= password.Length - 1; i++)
					{
						...
						{
							password[i] = Strings.Chr((int) (PCS.Math.Common.RandomBetween(48, 57)));
							digits++;
						}
						else if (upperCase < minSpecialChars)
						{
							password[i] = Strings.Chr((int) (PCS.Math.Common.RandomBetween(65, 90)));
							upperCase++;
						}
						else
						{
							password[i] = Strings.Chr((int) (PCS.Math.Common.RandomBetween(97, 122)));
						}
					}
					...
					for (int i = 0; i <= password.Length - 1; i++)
					{
						swapIndex = (int) (PCS.Math.Common.RandomBetween(0, password.Length - 1));
			...
			private SqlConnection GetDatabaseConnection()
			{
				...
				try
				{
					switch (m_securityServer)
					{
						case SecurityServer.Development:
							connection = new SqlConnection(PCS.Security.Cryptography.Common.Decrypt(DevConnectionString));
							break;
						case SecurityServer.Acceptance:
							connection = new SqlConnection(PCS.Security.Cryptography.Common.Decrypt(AcpConnectionString));
							break;
						case SecurityServer.Production:
							connection = new SqlConnection(PCS.Security.Cryptography.Common.Decrypt(PrdConnectionString));
							break;
					}
				...
				catch (SqlException)
				{
					...
					connection = new SqlConnection(PCS.Security.Cryptography.Common.Decrypt(BakConnectionString));
					connection.Open();
				}
				catch (Exception)
				{
