	...
	namespace Radius
	{
		...
		public class RadiusClient : IDisposable
		{
			...
			public string ServerName
			{
				get
				{
					return TVA.Text.Common.ParseKeyValuePairs(m_udpClient.ConnectionString)["server"];
				}
				set
				{
					CheckDisposed();
					...
					{
						Dictionary<string, string> parts = TVA.Text.Common.ParseKeyValuePairs(m_udpClient.ConnectionString);
						parts("server") = value;
						m_udpClient.ConnectionString = TVA.Text.Common.JoinKeyValuePairs(parts);
					}
					else
					{
			...
			public int ServerPort
			{
				get
				{
					return System.Convert.ToInt32(TVA.Text.Common.ParseKeyValuePairs(m_udpClient.ConnectionString)["remoteport"]);
				}
				set
				{
					CheckDisposed();
				...
				if (m_udpClient.WaitForConnection(- 1) && m_udpClient.IsConnected)
				{
					...
					for (int i = 1; i <= m_requestAttempts; i++)
					{
						...
						if (m_responseBytes != null)
						{
							...
							if (response.Identifier == request.Identifier && TVA.IO.Common.CompareBuffers(response.Authenticator, RadiusPacket.CreateResponseAuthenticator(m_sharedSecret, request, response)) == 0)
							{
