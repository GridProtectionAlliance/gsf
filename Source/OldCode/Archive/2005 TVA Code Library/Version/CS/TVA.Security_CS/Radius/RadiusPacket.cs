...
namespace TVA.Security
{
	namespace Radius
	{
		...
		public class RadiusPacket : IBinaryDataProvider, IBinaryDataConsumer
		{
			...
			public RadiusPacket()
			{
				...
				m_identifier = (byte) (TVA.Math.Common.RandomBetween(0, 255));
				m_authenticator = TVA.Common.CreateArray<byte>(16);
			...
			public byte[] BinaryImage
			{
				get
				{
					byte[] image = TVA.Common.CreateArray<byte>(BinaryLength);
			...
			public static byte[] CreateRequestAuthenticator(string sharedSecret)
			{
				...
				string inputString = TVA.Security.Cryptography.Common.GenerateKey() + sharedSecret;
			...
			public static byte[] CreateResponseAuthenticator(string sharedSecret, RadiusPacket requestPacket, RadiusPacket responsePacket)
			{
				...
				byte[] inputBuffer = TVA.Common.CreateArray<byte>(responseBytes.Length + sharedSecretBytes.Length);
			...
			public static byte[] EncryptPassword(string password, string sharedSecret, byte[] requestAuthenticator)
			{
				...
				if (password.Length <= 128)
				{
					...
					byte[] sharedSecretBytes = RadiusPacket.GetBytes(sharedSecret);
					byte[] md5HashInputBytes = TVA.Common.CreateArray<byte>(sharedSecretBytes.Length + 16);
					MD5CryptoServiceProvider md5Provider = new MD5CryptoServiceProvider();
					if (passwordBytes.Length % 16 == 0)
					{
						// Length of password is a multiple of 16.
						result = TVA.Common.CreateArray<byte>(passwordBytes.Length);
					}
					else
					{
						...
						result = TVA.Common.CreateArray<byte>(((passwordBytes.Length / 16) * 16) + 16);
					}
