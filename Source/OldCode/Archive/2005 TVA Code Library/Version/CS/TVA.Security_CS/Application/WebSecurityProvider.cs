...
using TVA.Security.Application.Controls;
...
namespace TVA.Security
{
	namespace Application
	{
		...
		/// </summary>
		[ToolboxBitmap(typeof(WebSecurityProvider))]public partial class WebSecurityProvider
		{
			...
			[Category("Configuration")]public Page Parent
			{
				get
				{
					return m_parent;
			...
			/// </remarks>
			[Category("Configuration")]public int InactivityTimeout
			{
				get
				{
			...
			protected override void HandleAccessGranted()
			{
				...
				if (m_parent != null)
				{
					if (m_inactivityTimeout > 0 && ! m_parent.ClientScript.IsClientScriptBlockRegistered("ActivityMonitor"))
					{
						...
						m_parent.ClientScript.RegisterClientScriptBlock(@TVA.Security.Cryptography.Common.GetType(), "ActivityMonitor", with_1.ToString());
					}
				}
				else
				{
			...
			protected override string GetUsername()
			{
				...
				if (m_parent != null)
				{
					...
					return TVA.Security.Cryptography.Common.Decrypt(username);
				}
				else
				{
			...
			protected override string GetPassword()
			{
				...
				if (m_parent != null)
				{
					...
					return TVA.Security.Cryptography.Common.Decrypt(password);
				}
				else
				{
			...
			private void WebSecurityProvider_AfterLogin(object sender, System.EventArgs e)
			{
				...
				if (m_parent != null)
				{
					// We'll cache the security data for subsequent uses here instead of the AccessGranted phase in order
					// to allow the implementer to stop the login process before it completes.
