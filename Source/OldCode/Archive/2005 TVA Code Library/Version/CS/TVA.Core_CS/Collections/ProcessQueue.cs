...
namespace TVA
{
	namespace Collections
	{
		...
		public class ProcessQueue<T> : IList<T>, ICollection, IDisposable
		{
			...
			private ItemProcessedEventHandler ItemProcessedEvent;
			
			public event ItemProcessedEventHandler ItemProcessed
			{
				add
				{
					ItemProcessedEvent = (ItemProcessedEventHandler) System.Delegate.Combine(ItemProcessedEvent, value);
				}
				remove
				{
					ItemProcessedEvent = (ItemProcessedEventHandler) System.Delegate.Remove(ItemProcessedEvent, value);
				}
			}
			...
			private ItemsProcessedEventHandler ItemsProcessedEvent;
			
			public event ItemsProcessedEventHandler ItemsProcessed
			{
				add
				{
					ItemsProcessedEvent = (ItemsProcessedEventHandler) System.Delegate.Combine(ItemsProcessedEvent, value);
				}
				remove
				{
					ItemsProcessedEvent = (ItemsProcessedEventHandler) System.Delegate.Remove(ItemsProcessedEvent, value);
				}
			}
			...
			private ItemTimedOutEventHandler ItemTimedOutEvent;
			
			public event ItemTimedOutEventHandler ItemTimedOut
			{
				add
				{
					ItemTimedOutEvent = (ItemTimedOutEventHandler) System.Delegate.Combine(ItemTimedOutEvent, value);
				}
				remove
				{
					ItemTimedOutEvent = (ItemTimedOutEventHandler) System.Delegate.Remove(ItemTimedOutEvent, value);
				}
			}
			...
			private ItemsTimedOutEventHandler ItemsTimedOutEvent;
			
			public event ItemsTimedOutEventHandler ItemsTimedOut
			{
				add
				{
					ItemsTimedOutEvent = (ItemsTimedOutEventHandler) System.Delegate.Combine(ItemsTimedOutEvent, value);
				}
				remove
				{
					ItemsTimedOutEvent = (ItemsTimedOutEventHandler) System.Delegate.Remove(ItemsTimedOutEvent, value);
				}
			}
			...
			/// </summary>
			protected ProcessQueue(ProcessItemFunctionSignature processItemFunction, CanProcessItemFunctionSignature canProcessItemFunction, double processInterval, int maximumThreads, int processTimeout, bool requeueOnTimeout, bool requeueOnException) : this(processItemFunction, null, canProcessItemFunction, new List<T>(), processInterval, maximumThreads, processTimeout, requeueOnTimeout, requeueOnException)
			{
			...
			/// </summary>
			protected ProcessQueue(ProcessItemsFunctionSignature processItemsFunction, CanProcessItemFunctionSignature canProcessItemFunction, double processInterval, int maximumThreads, int processTimeout, bool requeueOnTimeout, bool requeueOnException) : this(null, processItemsFunction, canProcessItemFunction, new List<T>(), processInterval, maximumThreads, processTimeout, requeueOnTimeout, requeueOnException)
			{
			...
			protected ProcessQueue(ProcessItemFunctionSignature processItemFunction, ProcessItemsFunctionSignature processItemsFunction, CanProcessItemFunctionSignature canProcessItemFunction, IList<T> processQueue, double processInterval, int maximumThreads, int processTimeout, bool requeueOnTimeout, bool requeueOnException)
			{
				...
				else
				{
					...
					m_processTimer.Elapsed += new System.Timers.ElapsedEventHandler(m_processTimer_Elapsed);
			...
			public virtual string Status
			{
				get
				{
					...
					with_1.Append("       Queue processing is: ");
					if (m_enabled)
					{
					...
					with_1.AppendLine();
					with_1.Append("  Current processing state: ");
					if (Processing)
					{
						with_1.Append("Executing");
					}
					else
					{
						with_1.Append("Idle");
					...
					with_1.AppendLine();
					with_1.Append("       Processing interval: ");
					if (m_processingIsRealTime)
					{
						with_1.Append("Real-time");
					}
					else
					{
						with_1.Append(ProcessInterval);
						with_1.Append(" milliseconds");
					}
					with_1.AppendLine();
					with_1.Append("        Processing timeout: ");
					if (m_processTimeout == Timeout.Infinite)
					{
						with_1.Append("Infinite");
					}
					else
					{
						with_1.Append(m_processTimeout);
						with_1.Append(" milliseconds");
					}
					with_1.AppendLine();
					with_1.Append("      Queue threading mode: ");
					if (ThreadingMode == QueueThreadingMode.Asynchronous)
					{
						with_1.Append("Asynchronous - ");
						with_1.Append(m_maximumThreads);
						with_1.Append(" maximum threads");
					}
					else
					{
						with_1.Append("Synchronous");
					}
					with_1.AppendLine();
					with_1.Append("    Queue processing style: ");
					if (ProcessingStyle == QueueProcessingStyle.OneAtATime)
					{
						with_1.Append("One at a time");
					}
					else
					{
						with_1.Append("Many at once");
					}
					with_1.AppendLine();
					with_1.Append("    Total process run time: ");
					with_1.Append(TVA.DateTime.Common.SecondsToText(RunTime));
					with_1.AppendLine();
					with_1.Append("      Total active threads: ");
					with_1.Append(m_threadCount);
					with_1.AppendLine();
					with_1.Append("   Queued items to process: ");
					with_1.Append(Count);
					with_1.AppendLine();
					with_1.Append("     Items being processed: ");
					with_1.Append(m_itemsProcessing);
					with_1.AppendLine();
					with_1.Append("     Total items processed: ");
					with_1.Append(m_itemsProcessed);
					with_1.AppendLine();
			...
			protected void RaiseItemsProcessed(T[] items)
			{
				...
				if (ItemsProcessedEvent != null)
					ItemsProcessedEvent(items);
			...
			protected void RaiseItemTimedOut(T item)
			{
				...
				if (ItemTimedOutEvent != null)
					ItemTimedOutEvent(item);
			...
			protected void RaiseItemsTimedOut(T[] items)
			{
				...
				if (ItemsTimedOutEvent != null)
					ItemsTimedOutEvent(items);
			...
			private class ProcessThread
			{
				...
				public ProcessThread(ProcessQueue<T> parent, T item)
				{
					...
					m_parent = parent;
					m_item = item;
					m_thread = new Thread(new System.Threading.ThreadStart(ProcessItem));
					m_thread.Start();
				...
				public ProcessThread(ProcessQueue<T> parent, T[] items)
				{
					...
					m_parent = parent;
					m_items = items;
					m_thread = new Thread(new System.Threading.ThreadStart(ProcessItems));
					m_thread.Start();
				...
				try
				{
					...
					if (processingItem)
					{
						...
						else
						{
							...
							object with_1 = new ProcessThread(this, nextItem);
							if (! with_1.WaitUntil(m_processTimeout))
							{
								// Notifies user of process timeout, in case they want to do anything special.
								if (ItemTimedOutEvent != null)
									ItemTimedOutEvent(nextItem);
			...
			private void ProcessNextItems()
			{
				...
				finally
				{
					...
					{
						Interlocked.Decrement(ref m_threadCount);
						Interlocked.Add(ref m_itemsProcessing, System.Convert.ToInt32(- nextItems.Length));
					}
				}
			...
			public virtual int IndexOf(T item, int index, int count)
			{
				...
				lock(m_processQueue)
				{
					...
					if (processQueue == null)
					{
						...
						int foundindex = - 1;
						Generic.Comparer<T> comparer = Generic.Comparer<T>.Default;
			...
			public virtual T[] ToArray()
			{
				...
				lock(m_processQueue)
				{
					...
					{
						// We manually implement this feature, if process queue is not a List(Of T).
						T[] items = TVA.Common.CreateArray<T>(m_processQueue.Count);
			...
			public IEnumerator IEnumerableGetEnumerator()
			{
				...
				return ((System.Collections.IEnumerable) m_processQueue).GetEnumerator();
