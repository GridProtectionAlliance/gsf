		...
		namespace Cryptography
		{
			...
			public sealed class Common
			{
				...
				public static void Encrypt(Stream inStream, Stream outStream, byte[] key, byte[] IV, EncryptLevel strength, ProgressEventHandler progressHandler)
				{
					...
					byte[] inBuffer = TVA.Common.CreateArray<byte>(BufferSize);
					byte[] outBuffer;
					byte[] lengthBuffer;
					int read;
					long total;
				...
				public static void Decrypt(Stream inStream, Stream outStream, byte[] key, byte[] IV, EncryptLevel strength, ProgressEventHandler progressHandler)
				{
					...
					while (read > 0)
					{
						...
						if (size > 0)
						{
							...
							inBuffer = TVA.Common.CreateArray<byte>(size);
				...
				public static string GenerateKey()
				{
					...
					with_1.Append(System.DateTime.UtcNow.Ticks);
					for (x = 1; x <= 50; x++)
					{
					...
					with_1.Append(System.DateTime.Now.ToString().Replace("/", "¡¤¥").Replace(" ", "°"));
				...
				public static Int24 GetSeedFromKey(string key)
				{
					...
					byte[] seedBytes = TVA.Common.CreateArray<byte>(3);
					int delimeter1;
					int delimeter2;
					string code = "";
					int value;
