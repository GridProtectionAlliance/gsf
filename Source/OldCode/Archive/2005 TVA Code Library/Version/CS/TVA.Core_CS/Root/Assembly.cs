...
namespace TVA
{
	public class Assembly
	{
		...
		private static Hashtable m_assemblyCache;
		...
		public string ComCompatibleVersion
		{
			get
			{
				ComCompatibleVersionAttribute with_1 = ((ComCompatibleVersionAttribute) (GetCustomAttribute(typeof(ComCompatibleVersionAttribute))));
				return with_1.MajorVersion.ToString() + "." + with_1.MinorVersion.ToString() + "." + with_1.RevisionNumber.ToString() + "." + with_1.BuildNumber.ToString();
			}
		}
		...
		public string TypeLibVersion
		{
			get
			{
				TypeLibVersionAttribute with_1 = ((TypeLibVersionAttribute) (GetCustomAttribute(typeof(TypeLibVersionAttribute))));
				return with_1.MajorVersion.ToString() + "." + with_1.MinorVersion.ToString();
			}
		}
		...
		public System.Collections.Specialized.NameValueCollection GetAttributes()
		{
			
			System.Collections.Specialized.NameValueCollection assemblyAttributes = new System.Collections.Specialized.NameValueCollection();
			...
			if (resourceAssembly == null)
			{
				...
				foreach (string name in System.Reflection.Assembly.GetEntryAssembly().GetManifestResourceNames())
				{
					...
					if (string.Compare(Path.GetFileNameWithoutExtension(name), EntryAssembly.RootNamespace+ "." + shortName, true) == 0)
					{
						...
						System.IO.Stream with_1 = System.Reflection.Assembly.GetEntryAssembly().GetManifestResourceStream(name);
						byte[] buffer = TVA.Common.CreateArray<byte>(with_1.Length);
						with_1.Read(buffer, 0, with_1.Length);
						with_1.Close();
