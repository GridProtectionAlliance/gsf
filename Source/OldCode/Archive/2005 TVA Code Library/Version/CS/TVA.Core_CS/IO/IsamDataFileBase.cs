...
namespace TVA
{
	namespace IO
	{
		...
		public abstract partial class IsamDataFileBase<T> : IPersistSettings, ISupportInitialize where T : IBinaryDataProvider
		{
			...
			[Browsable(false)]public bool IsCorrupt
			{
				get
				{
					if (IsOpen)
			...
			[Browsable(false)]public bool IsSynchronized
			{
				get
				{
			...
			[Browsable(false)]public long MemoryUsage
			{
				get
				{
			...
			[Browsable(false)]public int InMemoryRecordCount
			{
				get
				{
					int recordCount = 0;
			...
			[Browsable(false)]public int PersistedRecordCount
			{
				get
				{
					if (IsOpen)
			...
			[Browsable(false)]public abstract int RecordSize{
				get;
			}
			...
			private T ReadFromDisk(int recordID)
			{
				...
				byte[] binaryImage = TVA.Common.CreateArray<byte>(RecordSize);
				lock(m_fileStream)
				{
