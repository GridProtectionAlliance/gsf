...
namespace TVA
{
	namespace IO
	{
		...
		public sealed class FilePath
		{
			...
			[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Unicode)]private struct NETRESOURCE
			{
				public int dwScope;
				public int dwType;
			...
			public static string GetFilePatternRegularExpression(string fileSpec)
			{
				...
				if (m_fileNameCharPattern == null)
				{
					...
					with_1.Append("[^");
					with_1.Append(TVA.Text.Common.EncodeRegexChar(Path.DirectorySeparatorChar));
					with_1.Append(TVA.Text.Common.EncodeRegexChar(Path.AltDirectorySeparatorChar));
					with_1.Append(TVA.Text.Common.EncodeRegexChar(Path.PathSeparator));
					with_1.Append(TVA.Text.Common.EncodeRegexChar(Path.VolumeSeparatorChar));
					
					foreach (char c in Path.GetInvalidPathChars())
					{
						with_1.Append(TVA.Text.Common.EncodeRegexChar(c));
					}
					
					with_1.Append("]");
					m_fileNameCharPattern = with_1.ToString();
				}
			...
			public static long GetFileLength(string fileName)
			{
				...
				try
				{
					System.IO.FileInfo with_1 = new FileInfo(fileName);
					return with_1.Length;
				}
				catch
				{
					return - 1;
			...
			public static string GetTempFile(bool useTempPath, bool createZeroLengthFile, string fileExtension)
			{
				...
				{
					string tempFile = GetTempFilePath() + GetTempFileName(fileExtension);
					
					System.IO.FileStream with_1 = File.Create(tempFile);
					with_1.Close();
				...
				{
					string tempFile = GetApplicationPath() + GetTempFileName(fileExtension);
					
					System.IO.FileStream with_2 = File.Create(tempFile);
					with_2.Close();
			...
			public static string GetApplicationPath()
			{
				...
				return JustPath(System.Reflection.Assembly.GetEntryAssembly().Location.Trim());
			...
			if (fileName.Length > length)
			{
				...
				if (justName.Length == fileName.Length)
				{
					...
					if (trimName.Length > 8)
					{
						...
						float offset = (length - justExtension.Length- 3) / 2;
						return trimName.Substring(0, (int) (System.Math.Ceiling(offset))) + "..." + trimName.Substring(trimName.Length - System.Math.Floor(offset) + 1) + justExtension;
					}
					else
					{
				...
				{
					// Just file name part exceeds length. Recurses into function without path.
		...
		public static void WaitForReadLock(string fileName, double secondsToWait)
		{
			...
			double startTime = TVA.DateTime.Common.SystemTimer;
			...
			while (true)
			{
				...
				if (secondsToWait > 0)
				{
					if (TVA.DateTime.Common.SystemTimer > startTime + secondsToWait)
					{
