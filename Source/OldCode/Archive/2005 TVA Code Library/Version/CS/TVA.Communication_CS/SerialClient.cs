...
{
	public partial class SerialClient
	{
		...
		public SerialClient(string connectionString) : this()
		{
		...
		public override void Connect()
		{
			...
			if (base.Enabled && ! base.IsConnected && ValidConnectionString(ConnectionString))
			{
				...
				m_serialClient.BaudRate = Convert.ToInt32(m_connectionData("baudrate"));
				m_serialClient.DataBits = Convert.ToInt32(m_connectionData("databits"));
				...
				if (m_connectionData.ContainsKey("dtrenable"))
				{
					m_serialClient.DtrEnable = Convert.ToBoolean(m_connectionData("dtrenable"));
				}
				if (m_connectionData.ContainsKey("rtsenable"))
				{
					m_serialClient.RtsEnable = Convert.ToBoolean(m_connectionData("rtsenable"));
				}
				...
				#else
				m_connectionThread = new Thread(new System.Threading.ThreadStart(ConnectToPort));
				#endif
		...
		protected override bool ValidConnectionString(string connectionString)
		{
			...
			if (! string.IsNullOrEmpty(connectionString))
			{
				...
				if (m_connectionData.ContainsKey("port") && m_connectionData.ContainsKey("baudrate") && m_connectionData.ContainsKey("parity") && m_connectionData.ContainsKey("stopbits") && m_connectionData.ContainsKey("databits"))
				{
					return true;
				}
				else
				{
					...
					with_1.AppendLine();
					with_1.Append("   Port=[Name of the COM port]; BaudRate=[9600|4800|2400|1200]; Parity=[None|Odd|Even|Mark|Space]; StopBits=[None|One|Two|OnePointFive]; DataBits=[Number of data bits per byte]");
		...
		private void m_serialClient_DataReceived(object sender, System.IO.Ports.SerialDataReceivedEventArgs e)
		{
			...
			for (int x = 1; x <= Convert.ToInt32(System.Math.Ceiling(m_serialClient.BytesToRead / m_buffer.Length)); x++)
			{
