...
namespace TVA.Communication
{
	public partial class TcpClient
	{
		...
		public TcpClient(string connectionString) : this()
		{
		...
		protected override void SendPreparedData(byte[] data)
		{
			...
			if (base.Enabled && base.IsConnected)
			{
				...
				{
					data = EncryptData(data, m_tcpClient.Passphrase, base.Encryption);
				}
		...
		protected override bool ValidConnectionString(string connectionString)
		{
			...
			if (! string.IsNullOrEmpty(connectionString))
			{
				...
				if (m_connectionData.ContainsKey("server") && ! string.IsNullOrEmpty(m_connectionData("server")) && m_connectionData.ContainsKey("port") && ValidPortNumber(m_connectionData("port")))
				{
				...
				else
				{
					...
					with_1.AppendLine();
					with_1.Append("   Server=Server name or IP; Port=Server port number");
			...
			while (base.MaximumConnectionAttempts == - 1 || connectionAttempts < base.MaximumConnectionAttempts)
			{
				try
				{
					...
					if (m_tcpClient.Client.Connected) // Client connected to the server successfully.
					{
						...
						#if ThreadTracking
						TVA.Threading.ManagedThread with_1 = new ManagedThread(ReceiveServerData);
						with_1.Name = "TVA.Communication.TcpClient.ReceiveServerData() [" + m_tcpClient.ID.ToString() + "]";
						#else
						System.Threading.Thread with_2 = new Thread(new System.Threading.ThreadStart(ReceiveServerData));
						#endif
						.Start();
				...
				else
				{
					...
					while (true)
					{
						if (payloadSize == - 1)
						{
							...
							m_tcpClient.DataBuffer = TVA.Common.CreateArray<byte>(PayloadAwareHelper.PayloadHeaderSize);
						}
						...
						try
						{
							...
							if (payloadSize == - 1)
							{
								...
								if (payloadSize != - 1 && payloadSize <= CommunicationClientBase.MaximumDataSize)
								{
									...
									m_tcpClient.DataBuffer = TVA.Common.CreateArray<byte>(payloadSize);
								}
							}
							else
							{
		...
		private void ProcessReceivedServerData(byte[] data)
		{
			...
			else
			{
				...
				if (base.SecureSession)
				{
					data = DecryptData(data, m_tcpClient.Passphrase, base.Encryption);
				}
