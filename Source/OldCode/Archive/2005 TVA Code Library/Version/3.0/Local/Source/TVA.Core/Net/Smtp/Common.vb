        ...
        Public Shared Sub SendMail(ByVal from As String, ByVal toRecipients As String, ByVal ccRecipients As String, _
            ...
            With emailMessage
                ...
                If Not String.IsNullOrEmpty(attachments) Then
                    ...
                    For Each attachment As String In attachments.Replace(" ", "").Split(New Char() {";"c, ","c})
                        ...
                        .Attachments.Add(data)  ' Attaches the file.
                    Next
                End If
            ...
            With New SmtpClient()
                If smtpServer IsNot Nothing Then
                    .Host = smtpServer  ' Uses the specified SMTP server for sending the e-mail.
                Else
                    .Host = DefaultSmtpServer   ' Uses the default SMTP server for sending the e-mail.
                End If
                .Send(emailMessage) ' Sends the e-mail message.
            End With
