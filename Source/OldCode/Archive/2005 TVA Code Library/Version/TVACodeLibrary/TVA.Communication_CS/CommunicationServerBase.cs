...
{
	[ToolboxBitmap(typeof(CommunicationServerBase)), DefaultEvent("ReceivedClientData")]public abstract partial class CommunicationServerBase : ICommunicationServer, IPersistSettings, ISupportInitialize
	{
		...
		/// </summary>
		[Description("Occurs when the server is started."), Category("Server")]public delegate void ServerStartedEventHandler(object sender, System.EventArgs e);
		private ServerStartedEventHandler ServerStartedEvent;
		
		public event ServerStartedEventHandler ServerStarted
		{
			add
			{
				ServerStartedEvent = (ServerStartedEventHandler) System.Delegate.Combine(ServerStartedEvent, value);
			}
			remove
			{
				ServerStartedEvent = (ServerStartedEventHandler) System.Delegate.Remove(ServerStartedEvent, value);
			}
		}
		...
		/// </summary>
		[Description("Occurs when the server is stopped."), Category("Server")]public delegate void ServerStoppedEventHandler(object sender, System.EventArgs e);
		private ServerStoppedEventHandler ServerStoppedEvent;
		
		public event ServerStoppedEventHandler ServerStopped
		{
			add
			{
				ServerStoppedEvent = (ServerStoppedEventHandler) System.Delegate.Combine(ServerStoppedEvent, value);
			}
			remove
			{
				ServerStoppedEvent = (ServerStoppedEventHandler) System.Delegate.Remove(ServerStoppedEvent, value);
			}
		}
		...
		/// </summary>
		[Description("Occurs when an exception is encountered while starting up the server."), Category("Server")]public delegate void ServerStartupExceptionEventHandler(object sender, GenericEventArgs<Exception> e);
		private ServerStartupExceptionEventHandler ServerStartupExceptionEvent;
		
		public event ServerStartupExceptionEventHandler ServerStartupException
		{
			add
			{
				ServerStartupExceptionEvent = (ServerStartupExceptionEventHandler) System.Delegate.Combine(ServerStartupExceptionEvent, value);
			}
			remove
			{
				ServerStartupExceptionEvent = (ServerStartupExceptionEventHandler) System.Delegate.Remove(ServerStartupExceptionEvent, value);
			}
		}
		...
		/// </summary>
		[Description("Occurs when a client is connected to the server."), Category("Client")]public delegate void ClientConnectedEventHandler(object sender, GenericEventArgs<Guid> e);
		private ClientConnectedEventHandler ClientConnectedEvent;
		
		public event ClientConnectedEventHandler ClientConnected
		{
			add
			{
				ClientConnectedEvent = (ClientConnectedEventHandler) System.Delegate.Combine(ClientConnectedEvent, value);
			}
			remove
			{
				ClientConnectedEvent = (ClientConnectedEventHandler) System.Delegate.Remove(ClientConnectedEvent, value);
			}
		}
		...
		/// </summary>
		[Description("Occurs when a client is disconnected from the server."), Category("Client")]public delegate void ClientDisconnectedEventHandler(object sender, GenericEventArgs<Guid> e);
		private ClientDisconnectedEventHandler ClientDisconnectedEvent;
		
		public event ClientDisconnectedEventHandler ClientDisconnected
		{
			add
			{
				ClientDisconnectedEvent = (ClientDisconnectedEventHandler) System.Delegate.Combine(ClientDisconnectedEvent, value);
			}
			remove
			{
				ClientDisconnectedEvent = (ClientDisconnectedEventHandler) System.Delegate.Remove(ClientDisconnectedEvent, value);
			}
		}
		...
		/// </summary>
		[Description("Occurs when data is received from a client."), Category("Data")]public delegate void ReceivedClientDataEventHandler(object sender, GenericEventArgs<IdentifiableItem<Guid, byte[]>> e);
		private ReceivedClientDataEventHandler ReceivedClientDataEvent;
		
		public event ReceivedClientDataEventHandler ReceivedClientData
		{
			add
			{
				ReceivedClientDataEvent = (ReceivedClientDataEventHandler) System.Delegate.Combine(ReceivedClientDataEvent, value);
			}
			remove
			{
				ReceivedClientDataEvent = (ReceivedClientDataEventHandler) System.Delegate.Remove(ReceivedClientDataEvent, value);
			}
		}
		...
		[Description("The data that is required by the server to initialize."), Category("Configuration")]public virtual string ConfigurationString
		{
			get
			{
		...
		[Description("The maximum number of clients that can connect to the server. Set MaximumClients = -1 for infinite client connections."), Category("Configuration"), DefaultValue(typeof(int), "-1")]public virtual int MaximumClients
		{
			get
			{
		...
		[Description("Indicates whether the data exchanged between the server and clients will be encrypted using a private session passphrase."), Category("Security"), DefaultValue(typeof(bool), "False")]public virtual bool SecureSession
		{
			...
			set
			{
				if ((! value) || (value && m_handshake && m_encryption != Security.Cryptography.EncryptLevel.None))
				{
		...
		[Description("The passpharse that the clients must provide for authentication during the handshake process."), Category("Security"), DefaultValue(typeof(string), "")]public virtual string HandshakePassphrase
		{
			get
			{
		...
		[Description("The maximum number of bytes that can be received at a time by the server from the clients."), Category("Data"), DefaultValue(typeof(int), "8192")]public virtual int ReceiveBufferSize
		{
			get
			{
		...
		/// </para>
		/// </remarks>
		[Description("The encryption level to be used for encrypting the data exchanged between the server and clients."), Category("Data"), DefaultValue(typeof(TVA.Security.Cryptography.EncryptLevel), "None")]public virtual TVA.Security.Cryptography.EncryptLevel Encryption
		{
			get
			{
				return m_encryption;
			...
			set
			{
				if ((! m_secureSession) || (m_secureSession && value != Security.Cryptography.EncryptLevel.None))
				{
					m_encryption = value;
				}
				else
		...
		[Description("The compression level to be used for compressing the data exchanged between the server and clients."), Category("Data"), DefaultValue(typeof(TVA.IO.Compression.CompressLevel), "NoCompression")]public virtual TVA.IO.Compression.CompressLevel Compression
		{
			get
			{
		...
		[Description("Indicates whether the server is enabled."), Category("Behavior"), DefaultValue(typeof(bool), "True")]public virtual bool Enabled
		{
			get
			{
				return m_enabled;
		...
		[Browsable(false)]public ICommunicationServer This
		{
			get
			{
				return this;
		...
		[Browsable(false)]public virtual List<Guid> ClientIDs
		{
			get
			{
				return m_clientIDs;
		...
		[Browsable(false)]public virtual bool IsRunning
		{
			get
			{
				return m_isRunning;
		...
		[Browsable(false)]public virtual double RunTime
		{
			get
			{
		...
		public virtual void SendTo(Guid clientID, object serializableObject)
		{
			...
			SendTo(clientID, TVA.Serialization.GetBytes(serializableObject));
		...
		public virtual void Multicast(object serializableObject)
		{
			...
			Multicast(TVA.Serialization.GetBytes(serializableObject));
	...
	[Browsable(false)]public  string Status
	{
		get
		{
			...
			with_1.AppendLine();
			with_1.Append("              Server state: ");
			with_1.Append(TVA.Common.IIf(m_isRunning, "Running", "Not Running"));
			with_1.AppendLine();
			with_1.Append("            Server runtime: ");
			with_1.Append(TVA.DateTime.Common.SecondsToText(RunTime));
			with_1.AppendLine();
			with_1.Append("      Configuration string: ");
			with_1.Append(m_configurationString);
			with_1.AppendLine();
			with_1.Append("        Subscribed clients: ");
			lock(m_clientIDs)
			{
				with_1.Append(m_clientIDs.Count());
			}
			with_1.AppendLine();
			with_1.Append("           Maximum clients: ");
			with_1.Append(TVA.Common.IIf(m_maximumClients == - 1, "Infinite", m_maximumClients.ToString()));
			with_1.AppendLine();
	...
	public virtual void LoadSettings()
	{
		...
		try
		{
			...
			if (with_1.Count > 0)
			{
				ConfigurationString = with_1.Item("ConfigurationString").GetTypedValue(m_configurationString);
				ReceiveBufferSize = with_1.Item("ReceiveBufferSize").GetTypedValue(m_receiveBufferSize);
				MaximumClients = with_1.Item("MaximumClients").GetTypedValue(m_maximumClients);
	...
	protected virtual void OnServerStartupException(Exception e)
	{
		...
		if (ServerStartupExceptionEvent != null)
			ServerStartupExceptionEvent(this, new GenericEventArgs<Exception>(e));
	...
	protected virtual void OnClientConnected(Guid e)
	{
		...
		if (ClientConnectedEvent != null)
			ClientConnectedEvent(this, new GenericEventArgs<Guid>(e));
	...
	protected virtual void OnClientDisconnected(Guid e)
	{
		...
		if (ClientDisconnectedEvent != null)
			ClientDisconnectedEvent(this, new GenericEventArgs<Guid>(e));
	...
	protected virtual void OnReceivedClientData(IdentifiableItem<Guid, byte[]> e)
	{
		...
		if (ReceivedClientDataEvent != null)
			ReceivedClientDataEvent(this, new GenericEventArgs<IdentifiableItem<Guid, byte>>(e));
