	...
	namespace Services
	{
		...
		public sealed class Common
		{
			...
			public static DataTable BuildExportInfoTable(int recCount, string messageName)
			{
				...
				dr[0] = recCount;
				dr[1] = DateTime.Now;
				dr[2] = TVA.Configuration.Common.CategorizedSettings("WebServices.RefreshSchedule").Item(messageName + ".RefreshSchedule").Value;
				dr[3] = TVA.Configuration.Common.CategorizedSettings("WebServices.SourceDataTimeZone").Item(messageName + ".SourceDataTimeZone").Value;
			...
			public static DataTable BuildSourceInfoTable(string messageName)
			{
				...
				string[] sourceTables = TVA.Configuration.Common.CategorizedSettings("WebServices.SourceTables").Item(messageName + ".SourceTables").Value.Split(';');
