...
HtmlPage::HtmlPage(HtmlOutputDev* parent, GBool rawOrder, char *imgExtVal) {
  this->parent = parent;
  ...
  xyStrings = NULL;
  yxCur1 = yxCur2 = NULL;
  fonts=new HtmlFontAccu(parent->xml);
  links=new HtmlLinks();
  pageWidth=0;
  pageHeight=0;
  fontsPageMarker = 0;
...
void HtmlPage::coalesce() {
  ...
  while (str1 && (str2 = str1->yxNext)) {
    ...
    addLineBreak = !parent->noMerge && (fabs(str1->xMin - str2->xMin) < 0.4);
...
//printf("coalesce %d %d %f? ", str1->dir, str2->dir, d);
	...
	(!parent->complexMode || (hfont1->isEqualIgnoreBold(*hfont2))) && // in complex mode fonts must be the same, in other modes fonts do not metter
...
void HtmlPage::dump(FILE *f, int pageNum) 
{
  if (parent->complexMode)
  {
    if (parent->xml) dumpAsXML(f, pageNum);
    if (!parent->xml) dumpComplex(f, pageNum);  
  }
  else
  {
	...
	GBool outline) 
{
  ...
  // open file
  needClose = gFalse;
  pages = new HtmlPage(this, rawOrder, extension);
