...
namespace PCS.PhasorProtocols.FNet
{
    ...
    public class FrameParser : FrameParserBase<int>
    {
        ...
        /// </para>
        /// </remarks>
        protected override int ParseCommonHeader(byte[] buffer, int offset, int length, out ICommonHeader<int> commonHeader)
        {
            ...
            if (length > 0)
            {
                ...
                if (m_configurationFrame != null)
                {
                    ...
                    commonHeader = parsedFrameHeader;
                    commonHeader.State = new DataFrameParsingState(parsedLength, m_configurationFrame, DataCell.CreateNewCell);
