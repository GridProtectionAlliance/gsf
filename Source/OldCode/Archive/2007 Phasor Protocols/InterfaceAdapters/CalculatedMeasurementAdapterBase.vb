...
Public MustInherit Class CalculatedMeasurementAdapterBase
    ...
    Protected Overridable Sub ProcessStateChanged(ByVal processName As String, ByVal newState As ProcessState) Implements IServiceComponent.ProcessStateChanged

        ' We don't normally handle changes in process state - but dervived classes may choose to...

    End Sub

    Protected Overridable Sub ServiceStateChanged(ByVal newState As ServiceState) Implements IServiceComponent.ServiceStateChanged

        Select Case newState
            Case ServiceState.Paused
                ' Pause concentrator
                Me.Enabled = False
                UpdateStatus(String.Format("""{0}"" measurement calculation paused at the request of service manager...", Name))
