...
namespace GSF.TimeSeries.Transport
{
    ...
    public class ClientConnection : IProvideStatus, IDisposable
    {
        ...
        public bool RotateCipherKeys()
        {
            ...
            if ((DateTime.UtcNow.Ticks - m_lastCipherKeyUpdateTime).ToMilliseconds() >= 1000.0D)
            {
                try
                {
                    ...
                    m_parent.PublishStatusMessage(ConnectionID + " cipher keys rotated.");
                    return true;
                }
                catch (Exception ex)
                {
                    ...
                    m_parent.PublishStatusMessage("Failed to establish new cipher keys for {0}: {1}", ConnectionID, ex.Message);
                    return false;
                }
            }
            ...
            m_parent.PublishStatusMessage("WARNING: Cipher key rotation skipped for {0}, keys were already rotated within last second.", ConnectionID);
            return false;
        }
        ...
        private void m_dataChannel_ClientConnectingException(object sender, EventArgs<Exception> e)
        {
            ...
            m_parent.PublishProcessException(new InvalidOperationException(string.Format("Data channel exception occurred while sending client data to \"{0}\": {1}", m_connectionID, ex.Message), ex));
        }
        ...
        {
            m_parent.PublishStatusMessage("Data channel started.");
        }
        ...
        private void m_dataChannel_ServerStopped(object sender, EventArgs e)
        {
            ...
            {
                m_parent.PublishStatusMessage("Data channel stopped unexpectedly, restarting data channel...");
            ...
            {
                m_parent.PublishStatusMessage("Data channel stopped.");
            }
        }
        ...
        private void m_reconnectTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            try
            {
                m_parent.PublishStatusMessage("Attempting to restart data channel...");
                DataChannel = null;
                ...
                m_parent.PublishStatusMessage("Data channel successfully restarted.");
            }
            catch (Exception ex)
            {
                m_parent.PublishStatusMessage("Failed to restart data channel due to exception: {0}", ex.Message);
                m_reconnectTimer.Start();
            }
        }
