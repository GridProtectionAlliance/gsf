    ...
    public static class FilePath
    {
        ...
        public static bool IsValidFileName(string filePath)
        {
            ...
            try
            {
                ...
                if (!directory.IsNullOrWhiteSpace() && directory.IndexOfAny(Path.GetInvalidPathChars()) >= 0)
                    return false;

                // Check for invalid file name characters
                if (filename.IsNullOrWhiteSpace() || filename.IndexOfAny(Path.GetInvalidFileNameChars()) >= 0)
                    return false;

                // Recurse in to check validity of each directory name
                if (!directory.IsNullOrWhiteSpace() && !Path.GetFileName(directory).IsNullOrWhiteSpace())
